UpdateGitHub <- function(repo=getwd(), untracked=TRUE, stage=TRUE, commit=TRUE, pull=FALSE, push=TRUE) {
  ...
  packages <- c("git2r","rprojroot","tidyverse")
  ...
  username <- readline(prompt = "Usuario: ") #ALWAYS BE CAREFUL ABOUT STORING YOUR CREDENTIALS ON GITHUB!!
  password <- readline(prompt = "Contraseña: ") #ALWAYS BE CAREFUL ABOUT STORING YOUR CREDENTIALS ON GITHUB!!
  ...
  if (untracked == TRUE) {
    ...
    if (num > 0) {
      ...
      commit(message = paste(Sys.time() %>% format("%d-%m-%Y %H:%M:%S CET"), "Initial commit","by",username, sep = " - "))
  ...
  if (commit == TRUE) {
    if (!is.null(unlist(status()["staged"]))) {
      commit(message = paste(Sys.time() %>% format("%d-%m-%Y %H:%M:%S CET"), "Update","by",username, sep = " - ")) # Generic message, including timestamp.
